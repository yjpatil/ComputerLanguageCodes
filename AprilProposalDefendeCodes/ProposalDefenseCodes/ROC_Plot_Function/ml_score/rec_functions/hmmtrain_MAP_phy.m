function [guessTR,guessE,logliks] = hmmtrain_MAP_phy(seqs,guessTR,guessE,priorTR, priorE, varargin)
...
%             E(4,4)=E(4,4)+Ewgt(4);
    ...
    % ---------------------------------
    Esym=(guessE(2,2)+guessE(3,3))/2;
    Erem1=1-guessE(2,2);    Erem2=1-guessE(3,3);
    guessE(2,2)=Esym;               
    guessE(2,1)=guessE(2,1)/Erem1*(1-Esym);
    guessE(2,3)=guessE(2,3)/Erem1*(1-Esym);
    guessE(2,4)=guessE(2,4)/Erem1*(1-Esym);
    guessE(3,3)=Esym;
    guessE(3,1)=guessE(3,1)/Erem2*(1-Esym);
    guessE(3,2)=guessE(3,2)/Erem2*(1-Esym);
    guessE(3,4)=guessE(3,4)/Erem2*(1-Esym);
    %guessE(1,1)=1.2*guessE(2,2);    guessE(4,4)=guessE(2,2)/1.2;
    %totalEm = sum(guessE,2);
    %guessE = guessE./(repmat(totalEm,1,numEmissions));
