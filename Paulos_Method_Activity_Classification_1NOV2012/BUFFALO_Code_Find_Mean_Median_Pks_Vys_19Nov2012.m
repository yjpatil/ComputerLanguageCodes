...
while(I <= Size_List)
    ...
    while(Case_Value <= 2)
        ...
        if(Case_Value == 1)
            ...
            figure(1);hold on;
            title('Across 5Sub,Comparing pde for AS Peak Values Normalized using Median')
            H1_AS = gkde0(PtsAS(:,2));
            med_Jp_AS(I) = plot(H1_AS.x,H1_AS.f,'Color',[rand rand rand]);
            
            figure(2);hold on;            
            title('Across 5Sub,Comparing pde for VT Peak Values Normalized using Median')
            H1_VT = gkde0(PtsVT(:,2));
            med_Jp_VT(I) = plot(H1_VT.x,H1_VT.f,'Color',[rand rand rand]);            
            
            figure(3);hold on;
            title('Across 5Sub,Comparing pde for AS Valley Values Normalized using Median')
            H2_AS = gkde0(VtsAS(:,2));
            med_Jv_AS(I) = plot(H2_AS.x,H2_AS.f,'Color',[rand rand rand]);
            
            figure(4);hold on;
            title('Across 5Sub,Comparing pde for VT Valley Values Normalized using Median')
            H2_VT = gkde0(VtsVT(:,2));
        ...
        elseif(Case_Value == 2)
            ...
            figure(5);hold on;
            title('Across 5Sub,Comparing pde for VT Peak Values Normalized using Mean')            
            H3_VT = gkde0(PtsVT(:,2));
            mn_Jp_VT(I) = plot(H3_VT.x,H3_VT.f,'Color',[rand rand rand]);
            
            figure(6);hold on;
            title('Across 5Sub,Comparing pde for AS Peak Values Normalized using Mean')            
            H3_AS = gkde0(PtsAS(:,2));
            mn_Jp_AS(I) = plot(H3_AS.x,H3_AS.f,'Color',[rand rand rand]);
            
            figure(7);hold on;
            title('Across 5Sub,Comparing pde for VT Valley Values Normalized using Mean')
            H4_VT = gkde0(VtsVT(:,2));
            mn_Jv_VT(I) = plot(H4_VT.x,H4_VT.f,'Color',[rand rand rand]);
            
            figure(8);hold on;
            title('Across 5 Sub,Comparing pde for AS Valley Values Normalized using Mean')
            H4_AS = gkde0(VtsAS(:,2));
