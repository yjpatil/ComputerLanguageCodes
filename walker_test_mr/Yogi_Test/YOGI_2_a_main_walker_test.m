...
%---------------------------------------------------------
meys1 = rotateY(scale(cyl,L1/10, L1/12, L1/8),180);
meys1 = translate(meys1,0.7,0.4,L1+4.6);
meys1.facecolor = 'yellow';
meys2 = rotateY(scale(cyl,L1/10, L1/12, L1/8),180);
...
pathX = 14:-0.1:3;
pathY = 3*ones(size(pathX));

bump = 0;Total_bumps = 5;
flag1 = 1;flag2 = 1;flag3 = 1;flag4 = 0;
i = 1;

while(bump <= Total_bumps)
...
%for i = 1:10
   ...
   % ---------------------------------------------------------------------
    ...
    if(pathX(i) <= 3.3 && pathY(i) <= 17.3)
        angle_direct = 90;
        if(flag1 ==1)
            pathY = pathY(i):0.1:18;
            pathX = pathX(i)*ones(size(pathY));
            i = 1;
            flag1 = 0;
            flag4 = 1;
            bump = bump + 1;
    ...
    elseif(pathY(i) >= 17.3 && pathX(i) <= 14.9)
        angle_direct = 0;
        if(flag2 == 1)
            pathX = pathX(i):0.1:18;
            pathY = pathY(i)*ones(size(pathX));
            i = 1;
            flag2 = 0;
            flag1 = 1;
            bump = bump + 1;
    ...
    elseif(pathX(i) <= 22.3 && pathY(i) <= 4.3)%(pathY(i) <= 4.5)%% && pathX(i) >= 14.9) %%(pathY(i) <= 5.1 || pathX(i) >= 15)
        i
        angle_direct = 180;
        if(flag4 == 1)
            pathX = pathX(i):-0.1:3;
