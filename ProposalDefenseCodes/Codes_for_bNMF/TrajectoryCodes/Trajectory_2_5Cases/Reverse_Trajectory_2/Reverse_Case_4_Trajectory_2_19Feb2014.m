% Code to create TRAJECTORY For Square Rounds % For 2 Subjects %
...
StopSign = 42;  % <>----- "Important:: How many times you want the Target to bump at the end and take reverse turn"

carL = 1.2;
carW = 1.3;

% % ----- Initial assignment of the position of the subject ----- % %
s11 = [2.5,17.5,-90];%[X,Y,Angle w.r.t X]   % <>-------- Enter the Starting Point for 1st subject
computecarpoints(s11(1),s11(2),s11(3),'r',carL,carW);
hold on;
s22 = [2.5,17.5,-90];%[X,Y,Angle w.r.t X]  % <>-------- Enter the Starting Point for 2nd subject
...
while (s11(1) > 0.0000001 && s11(1) < 19.99995 && s11(2) > 0.000001 && s11(2) < 19.9999995)
    ...
    if (rem(i,SampleData)==0) % Plot if 'ith' iteration and SamplePlt value have 1 e.g (rem(11,10) = 1)
        ...
        if(XCoG1 >= 18.0 && YCoG1 <= 15)
            s11(3) = 90;
            [X1,Y1,XCoG1,YCoG1] = computecarpoints01(s11(1), s11(2), s11(3),'r',carL,carW,0);
            WalkCount1 = WalkCount1 + 1;
        elseif(XCoG1 <= 19 && YCoG1 >= 18.0)
            s11(3) = 180;
            [X1,Y1,XCoG1,YCoG1] = computecarpoints01(s11(1), s11(2), s11(3),'r',carL,carW,0);
            WalkCount1 = WalkCount1 + 1;
        elseif(XCoG1 <= 1.5 && YCoG1 <= 19.5)
            s11(3) = -90;
            [X1,Y1,XCoG1,YCoG1] = computecarpoints01(s11(1), s11(2), s11(3),'r',carL,carW,0);
            WalkCount1 = WalkCount1 + 1;
        elseif(XCoG1 <= 3.5 && YCoG1 <= 12.5)
            s11(3) = 0;
            [X1,Y1,XCoG1,YCoG1] = computecarpoints01(s11(1), s11(2), s11(3),'r',carL,carW,0);
            WalkCount1 = WalkCount1 + 1;
...
while (s22(1) > 0.0000001 && s22(1) < 19.99999995 && s22(2) > 0.0000001 && s22(2) < 19.9999995)
    ...
    if (rem(i,SampleData)==0)    
        ...
        CodeRec2(it2,:) = Code2';
        it2 = it2 + 1;
        if(XCoG2 >= 18.0 && YCoG2 <= 15)
            s22(3) = 90;
            [X2,Y2,XCoG2,YCoG2] = computecarpoints01(s22(1), s22(2), s22(3),'c',carL,carW,0);
            WalkCount2 = WalkCount2 + 1;
        elseif(XCoG2 <= 19 && YCoG2 >= 18.0)
            s22(3) = 180;
            [X2,Y2,XCoG2,YCoG2] = computecarpoints01(s22(1), s22(2), s22(3),'c',carL,carW,0);
            WalkCount2 = WalkCount2 + 1;
        elseif(XCoG2 <= 1.5 && YCoG2 <= 19.5)
            s22(3) = -90;
            [X2,Y2,XCoG2,YCoG2] = computecarpoints01(s22(1), s22(2), s22(3),'c',carL,carW,0);
            WalkCount2 = WalkCount2 + 1;
        elseif(XCoG2 <= 3.5 && YCoG2 <= 12.5)
            s22(3) = 0;
            [X2,Y2,XCoG2,YCoG2] = computecarpoints01(s22(1), s22(2), s22(3),'c',carL,carW,0);
            WalkCount2 = WalkCount2 + 1;
...
Rev_T2C4_Loc1 = Location1;Rev_T2C4_Loc2 = Location2;
Rev_T2C4_Dir1 = Direction1;Rev_T2C4_Dir2 = Direction2;
Rev_T2C4_Hist1 = Hist1;Rev_T2C4_Hist2 = Hist2;

clearvars -except Rev_T2C4_Loc1 Rev_T2C4_Loc2 Rev_T2C4_Dir1 Rev_T2C4_Dir2 Rev_T2C4_Hist1 Rev_T2C4_Hist2
