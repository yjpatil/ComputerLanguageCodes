...
% % %        legend([H2 H3(1) H4(1)],{'Volume Tidal' 'Start of Puff' 'End of Puff'});
       ...
       Table_Mean_AS(I,:) = [SUBJECT(I).Labs(LS).Mean_Std_AS(1,1) SUBJECT(I).Labs(LS).Mean_Std_AS(1,2) SUBJECT(I).Labs(LS).Mean_Std_AS(1,3) SUBJECT(I).Labs(LS).Mean_Std_AS(1,4) SUBJECT(I).Labs(LS).Mean_Std_AS(1,5)];
       
       Table_Mean_Std_AS(I,:) = [SUBJECT(I).Labs(LS).Mean_Std_AS(2,1) SUBJECT(I).Labs(LS).Mean_Std_AS(2,2) SUBJECT(I).Labs(LS).Mean_Std_AS(2,3) SUBJECT(I).Labs(LS).Mean_Std_AS(2,4) SUBJECT(I).Labs(LS).Mean_Std_AS(2,5)];
       ...
       if(Case == 1)  %% Only MEan Values %%
           ...
           Mean_mean_AS = SUBJECT(I).Labs(LS).Mean_Std_AS(1,4);Mean_std_AS = SUBJECT(I).Labs(LS).Mean_Std_AS(2,4); % <---- Mean of the Mean of Apnea
           L_Mean_AS = Mean_mean_AS - ((zvalue * Mean_std_AS)/sqrt(n)); % Lower Threshold for MEAN of APNEA  % THRESHOLD # 1 %
           U_Mean_AS = Mean_mean_AS + ((zvalue * Mean_std_AS)/sqrt(n)); % Upper Threshold for MEAN of APNEA  % THRESHOLD # 2 %
           ...
           Mean_mean_VT = SUBJECT(I).Labs(LS).Mean_Std_VT(1,4);Mean_std_VT = SUBJECT(I).Labs(LS).Mean_Std_VT(2,4); % <---- Mean of the Mean of Apnea
           L_Mean_VT = Mean_mean_VT - ((zvalue * Mean_std_VT)/sqrt(n)); % Lower Threshold for MEAN of APNEA  % THRESHOLD # 1 %
           U_Mean_VT = Mean_mean_VT + ((zvalue * Mean_std_VT)/sqrt(n)); % Upper Threshold for MEAN of APNEA  % THRESHOLD # 2 %
       elseif(Case == 2)  %% Only Median Values %%
           ...
           Mean_median_AS = SUBJECT(I).Labs(LS).Median_Std_AS(1,4);Median_std_AS = SUBJECT(I).Labs(LS).Median_Std_AS(2,4); % <---- Mean of the Mean of Apnea
           L_Median_AS = Mean_median_AS - ((zvalue * Median_std_AS)/sqrt(n)); % Lower Threshold for MEAN of APNEA  % THRESHOLD # 1 %
           U_Median_AS = Mean_median_AS + ((zvalue * Median_std_AS)/sqrt(n)); % Upper Threshold for MEAN of APNEA  % THRESHOLD # 2 %
           % For Tidal Volume Signal %
           ...
           Mean_median_VT = SUBJECT(I).Labs(LS).Median_Std_VT(1,4);Median_std_VT = SUBJECT(I).Labs(LS).Median_Std_VT(2,4); % <---- Mean of the Mean of Apnea
           L_Median_VT = Mean_median_VT - ((zvalue * Median_std_VT)/sqrt(n)); % Lower Threshold for MEAN of APNEA  % THRESHOLD # 1 %
           U_Median_VT = Mean_median_VT + ((zvalue * Median_std_VT)/sqrt(n)); % Upper Threshold for MEAN of APNEA  % THRESHOLD # 2 %
       end
       %% Now Change the Lab Sessiion Number %%        
       if( LS == 1 )
           LS = 2;
       elseif( LS == 2 )
           LS = 1; 
       ...
       while( pCount <= size(PtsAS,1)-5 ) %% For each peak of Air_Flow Signal
           ...
           Thresh1 = L_Mean_AS;  % Threshold 1 % Lower value of mean = This Threshold is the lower mean %
           Thresh2 = U_Mean_AS;  % Threshold 2 % Upper value of mean = This Threshold is the upper mean %
           
           Thresh3 = U_Max_AS + Thresh2; % Threshold 3 % Thresh3 = L_Max_AS + Thresh2; % Threshold BELOW which the Apnea Part Should Stay
       ...
       %------------------------ For Air Flow Signal ---------------------%
       if(LS == 1)
           LS1_Table_AS_Const(I,:) = [F1score*100 Prec*100 Rec*100 Counter_TP  (length(pf_score)-Counter_TP) Counter_FP length(pf_score) ((ts(length(ts))-ts(1))/60)];
       elseif(LS == 2)
           LS2_Table_AS_Const(I,:) = [F1score*100 Prec*100 Rec*100 Counter_TP  (length(pf_score)-Counter_TP) Counter_FP length(pf_score) ((ts(length(ts))-ts(1))/60)];
       end
       %---------------------- For Tidal Volume Signal -------------------%
       pCount = 3;
       Counter_TP = 0; Counter_FN = 0; Counter_FP = 0; % Initialize the values of TP, FN, FP
       while( pCount <= size(PtsVT,1)-5 ) %% For each peak of Air_Flow Signal
           ...
           Thresh1 = L_Mean_VT;  % Threshold 1 % Lower value of mean = This Threshold is the lower mean %
           Thresh2 = U_Mean_VT;  % Threshold 2 % Upper value of mean = This Threshold is the upper mean %
       ...
       %-------------------- For Tidal Volume Signal ---------------------%
       if(LS == 1)
           LS1_Table_VT_Const(I,:) = [F1score*100 Prec*100 Rec*100 Counter_TP  (length(pf_score)-Counter_TP) Counter_FP length(pf_score) ((ts(length(ts))-ts(1))/60)];
       elseif(LS == 2)
           LS2_Table_VT_Const(I,:) = [F1score*100 Prec*100 Rec*100 Counter_TP  (length(pf_score)-Counter_TP) Counter_FP length(pf_score) ((ts(length(ts))-ts(1))/60)];
       end
