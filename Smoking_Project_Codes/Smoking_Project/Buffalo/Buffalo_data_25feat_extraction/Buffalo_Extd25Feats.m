...
VT = Signals(:,2);        % Normalized VT signals with pre-processing applied
AS = Signals(:,3);        % Normalized AS signals with pre-processing applied
...
O = ones(size(puff_score));
plot(puff_score,O,'*y');
% *********************************************************************** %

% % % RC = (Data(:,3))/max(Data(:,3)); AB = (Data(:,4))/max(Data(:,4));    %% <----Dont need this step if you are using Paulos data
% % % 
...
[predict_label_L, accuracy_L, dec_values_L] = svmpredict(Test_L,Test_F, model_25feat);

LL = length(predict_label_L);

for i = 1 : 1 : LL
    if predict_label_L(i) == 1
        plot([SEtsPS(i,1) SEtsPS(i,2)],[1 1],'hc')
        grid on; hold on;
    else
    end
end
