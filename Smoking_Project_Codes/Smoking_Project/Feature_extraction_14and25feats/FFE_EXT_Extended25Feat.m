...
%list = char('sub_020','sub_021','sub_022','sub_023');
s = size(list);s = s(1,1);I = 1;   %% Iteration for subject (here, 1 <= I <= 20) %%

while(I <= s) %% do untill the total 20 subjects are not done !
...
%     fprintf('\nThe Subject# is:%d\n',I);
    ...
    clear ('L','ts');
    L = length(Data); ts = [1:L]/sf; ts = ts'; ts = ts-Activities(1,2); %% This part is to get actual time-stamping of data points
    
    clear ('Z','Z1','Z2');
    Z = zeros(L,1); Z1 = zeros(L,1); Z2 = zeros(L,1); %% These Three Matrices tries to remove the data points from the Non-Smoking Activities
    ZZ = zeros(L,1);
    clear ('ind1','ind2','indZZ1','indZZ2');
    
    % =============================== SUBtract/ADD extra 25 seconds ========================== %
    ind1 = find(Activities(11,1) - 20 <= ts & ts <= Activities(11,2) + 25); Z(ind1) = 1; Z1(ind1) = 1; %% Find the indices for Sit Smoking Act and put ones in Z and Z1
    % =============================== SUBtract/ADD extra 25 seconds ========================== %
    ind2 = find(Activities(13,1) - 20 <= ts & ts <= Activities(13,2) + 25); Z(ind2) = 1; Z2(ind2) = 1; %% Find the indices for Stand Smoking Act and put ones in Z and Z2
    % ======================================================================================== %
    %      REMOVE RINGING EFFECT in AS and VT signals        %
    indZZ1 = find(Activities(11,1) - 60 <= ts & ts <= Activities(11,2) + 60); ZZ(indZZ1) = 1;
    indZZ2 = find(Activities(13,1) - 60 <= ts & ts <= Activities(13,2) + 60); ZZ(indZZ2) = 1;
    % ======================================================================================== %
    clear ('PB','PS31','ind0','PS');    
    PB = (Data(:,1))/max(Data(:,1)); PB = PB .* Z; %% Get PB data only for Smoking Act
    
    PS31 = (Data(:,2))/max(Data(:,2)); PS31 = PS31.* Z; %% First Normalise PS data, then get data for only Smoking Act
    % PS31 = correct_signal(PS31);  %% Fix the signal first i.e.if(first/last)data points start with 1's fill them with 0's
    PS = merge_function1(PS31,ts,0.046,0.5,8,1.2); %% First Fix the signal given to this function
...
%     plot(ts,PS,'r');hold on;grid on;
    ...
    %ASa = (Data(:,3))/max(Data(:,3));  %% Normalize data for TRUE Air Flow signal
    RC = (Data(:,4))/max(Data(:,4));   %% Normalize data for Rib Cage signal
    AB = (Data(:,5))/max(Data(:,5));   %% Normalize data for Abdominal signal
    
    RC = RC .* ZZ; AB = AB .* ZZ; VT1 = (AB + RC)/2; %% Proces to get Volume Tidal Signal starts here %%
    
    ind3 = find(VT1 > 0); nvt1 = length(ind3); Mvt = (sum(VT1))/nvt1;  %% Procedure to find the mean-value of VT signal in Smoke Act%%
    
    clear ('VT','AS');
    VT = VT1 - Mvt; VT = -1 * VT; VT = VT + Mvt; VT = VT .* ZZ;   %% Inverted VT signal %%
    
    VT = idealfilter(VT,0.1,1000,sf); VT = VT .* ZZ; %% Filter the signal 
    VT = fastsmooth(VT,25,1,0); %% Apply Smoothing technique to VT signal
    
    ts1 = ts .* ZZ; %% This is done to apply differentiation on the signal for finding the AirFlow Signal    
    AS = diff(VT)./diff(ts1);  %% Do the time differentiation of AS
    AS = [AS;0];  %% Since Differentiation gives you 1 element less than previous data, add one zero in the end
    
    for i1 = 1:1:L   %% This "for_loop" tries to remove the NaN's from the AS signal, by substituting zero's
        if (ZZ(i1,1) == 0)
            AS(i1,1) = 0;
        else
            AS(i1,1) = AS(i1,1);
    ...
    AS = fastsmooth(AS,45,1,0);  %% Smooth the AS
    % =============================================================================== %
    ...
    % =============================================================================== %
    % ==================== Peaks and Valleys for VT and AS ===========================%
    clear ('LpVT','MpVT','LvVT','MvVT','LpAS','MpAS','LvAS','MvAS');
    [LpVT,MpVT] = peakfinder(VT,0.0099,-0.02,1); %% Find *Peaks* for the VT signal---->LpVT= (Location peak VT)signal;MpVT= (Magnitude peak VT)signal
    LpVT = LpVT/sf;   %% The location found are not actual time stamps, so divide them by sampling frequency
    LpVT = LpVT-(Activities(1,2));   %% Then bring them to original scale
    
    [LvVT,MvVT] = peakfinder(VT,0.0099,0.02,-1); %% Find *Valleys* for the VT signal---->LvVT= (Location valley VT)signal;MvVT= (Magnitude valley VT)signal
    LvVT = LvVT/sf;
    LvVT = LvVT-(Activities(1,2));    
    
    [LpAS,MpAS] = peakfinder(AS,0.0099,-0.02,1); %% Find *Peaks* for the AS signal---->LpAS= (Location peak AS)signal;MpAS= (Magnitude peak AS)signal
    LpAS = LpAS/sf;
    LpAS = LpAS-(Activities(1,2));
    
    [LvAS,MvAS] = peakfinder(AS,0.0099,0.02,-1); %% Find *Valleys* for the AS signal---->LvAS= (Location valley AS)signal;MvAS= (Magnitude valley AS)signal
    LvAS = LvAS/sf;
...
%     plot(LvVT,MvVT,'or');grid on;hold on; % Valleys for VT signal %
    ...
    % ===================  Simply to find the number of HG's in the Smoking Act ============================== %
    PS5 = PS.*Z1;
    [inttsPS1,y111] = intersections(ts,PS5,[0 ts(length(ts),1)],[0.1 0.1],1);%%inttsPS1 =>contains the Start and End Time stamps for 1st Smoke Act
    
    PS6 = PS.*Z2;
    [inttsPS2,y112] = intersections(ts,PS6,[0 ts(length(ts),1)],[0.1 0.1],1);%%inttsPS2 =>contains the Start and End Time stamps for 2nd Smoke Act
    ...
    SEtsPS = cat(1,SORTtsPS1,SORTtsPS2); %% Contains time stamps for START && END of PS for both Smoking Act
    EtsPS = cat(1,SORTtsPS1(:,2),SORTtsPS2(:,2));  %% Contains only time stamp for END of PS for both Somking Act
    
    clear ('PtsVT','VtsVT','PtsAS','VtsAS');
    %*************  IMPORTANT MATRICES FOR VT SIGNALS **************%
    PtsVT = [LpVT MpVT]; VtsVT = [LvVT MvVT]; %% *Peak_time-stamps_for_VT signals ||*Valley_time-stamps_for_VT signals
    %*************  IMPORTANT MATRICES FOR AS SIGNALS **************%
    PtsAS = [LpAS MpAS]; VtsAS = [LvAS MvAS]; %% *Peak_time-stamps_for_AS signals ||*Valley_time-stamps_for_VT signals
    %***************************************************************%
    ...
    for IT = 1 : 1 : length(EtsPS)
        ...
        if(ERR1 == 1)
            ...
            PEAK_AS_INSP(i,1) = PtsAS(IND,1);    %% Contains the TIME STAMPS for peaks of AS for PEAK_AS_INSP PARAMETER
            PEAK_AS_INSP(i,2) = PtsAS(IND,2);    %% Contains the PEAK AMPLITUDES for VALUES for PEAK_AS_INSP PARAMETER
            i = i + 1;
        end
    end
    clear 'IND'
    ...
    [tsintthp10AS,ampintthp10AS] = intersections(ts,AS,[0 ts(length(ts),1)],[thp10 thp10],1); %% time-stamps__intersection__threshold__plus__10__AS
    %--------------- NEGATIVE 10% MEDIAN THRESHOLD ------------------%
    thm10 = median(PEAK_AS_INSP(:,2)) * ( - 0.1);
    [tsintthm10AS,ampintthm10AS] = intersections(ts,AS,[0 ts(length(ts),1)],[thm10 thm10],1); %% time-stamps__intersection__threshold__minus__10__AS
    %============= INTERSECTION of ZERO line with VT ==============%
    [tsintZVT,ampintZVT] = intersections(ts,VT,[0 ts(length(ts),1)],[1e-7 1e-7],1); %% Time-stamps_Intersection_zero-line_&_VT
    %============= INTERSECTION of ZERO line with AS ==============%
    [tsintZAS,ampintZAS] = intersections(ts,AS,[0 ts(length(ts),1)],[1e-7 1e-7],1); %% Time-stamps_Intersection_zero-line_&_VT
...
%         fprintf('\nThe hand gesture #:%d\n',H);
        ...
        Subj_sa(I).label(H,1) = 1; %<==================== Label Assignment %%
