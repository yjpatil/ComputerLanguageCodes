function [p,H1,H2] = gkdebFig(x,p)
% GKDEB  Gaussian Kernel Density Estimation with Bounded Support
% 
% Usage:
% p = gkdeb(d) returns an estimate of pdf of the given random data d in p,
...
% Plot
if ~nargout
    figure(211);hold on; %% subplot(211) <<----- Changes made on 13 Jun 2013
    H1 = plot(p.x,p.pdf,'linewidth',2,'Color',color);
    grid
%     set(gca,'ylim',[0 max(p.pdf)*1.1])
    ylabel('f(x)')
    title('Estimated Probability Density Function');
    figure(212);hold on;%% subplot(212) <<----- Changes made on 13 Jun 2013 
    H2 = plot(p.x,p.cdf,'linewidth',2,'Color',color);
    ylabel('F(x)')
