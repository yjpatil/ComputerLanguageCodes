...
% %         end
        ...
        Levels = 9;
        %figure;hold on;
        [CodeAS] = EncodedSeq(ASts_temp,45,Levels); % The first argument is (1)Signal (AirFlow or TidalVolume), (2)Interval between succesive data points
        ...
        %figure;hold on;
        [CodeVT] = EncodedSeq(VTts_temp,45,Levels); % The first argument is (1)Signal (AirFlow or TidalVolume), (2)Interval between succesive data points
        % You can also form two separate sit and stand smk cells, by simply
        % comparing the ASpeak time that occur before the start of stand
        % smk event
        if(Label == +1)
...
%     hmmsmk.transmat(5,1) = 0;hmmsmk.transmat(5,2) = 0;hmmsmk.transmat(5,3) = 0;hmmsmk.transmat(5,4) = 0;
    ...
    [LL0, hmmsmk.prior, hmmsmk.transmat, hmmsmk.obsmat] = dhmm_em(ASdatasmk, ...
    ...
    for i = 1 : 1 : length(ASdatasmk)
        loglikSMK = dhmm_logprob(ASdatasmk{i},hmmsmk.prior, hmmsmk.transmat, hmmsmk.obsmat);        
