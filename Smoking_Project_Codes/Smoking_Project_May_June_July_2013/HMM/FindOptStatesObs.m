function [TableTest,TableTrain,LogStruct,OptTest,OptTrain,BestHMM] = FindOptStatesObs(VTts,ASts,Score,CodeAct,S_start,S_end,O_start,O_end,ActTrain,ActTest,Actlist,TimeInst,LogStruct)

OptTest = [0 0 -2000 2000 0 0 -2000 1];

OptTrain = [0 0 -2000 2000 0 0 -2000 1];
...
for States = S_start:1:S_end   % <<------ Include:: Step-size/Increment
    for Obs = O_start:1:O_end  % <<------ Include:: Step-size/Increment
        ...
        [LogLkhdAct1, HMMAct1] = GetHMM(CodeTrain,States,Obs,MaxIt);
        ...
        % ------- Update teh Tables ------ %
        TableTrain(I,:) = [States Obs mean(loglikSA) std(loglikSA) length(loglikSA) min(loglikSA) max(loglikSA) I];
        
        if(TableTrain(I,3) > OptTrain(1,3) && TableTrain(I,4) < OptTrain(1,4)) % If mean is less and std is less then this is teh Optimal
            OptTrain(1,:) = TableTrain(I,:);
        end
        
        
        TableTest(I,:) = [States Obs mean(loglikOA) std(loglikOA) length(loglikOA) min(loglikOA) max(loglikOA) I];
        
        if(TableTest(I,3) > OptTest(1,3) && TableTest(I,4) < OptTest(1,4)) % If mean is less and std is less then this is teh Optimal
            OptTest(1,:) = TableTest(I,:);
            BestHMM = [];
            BestHMM = HMMAct1;
        end
