function[Log1,Log2,Log3,EstLabel,SpecialLabel] = fTest3HMM_Ver1(Data,Labels,HMM1,HMM2,HMM3)
...
% HMMi (i = 1, 2, 3) = Different HMMs for Different Acts
...
for i = 1 : 1 : length(Names1)    % Now start copying data from the same field in struct 1 to struct 2 
    ...
    if(isempty(DataArray))        
        ...
        for j = 1 : 1 : L            
            ...
            Index = find( max([ Log1(i).loglik(j) Log2(i).loglik(j) Log3(i).loglik(j) ]) == [ Log1(i).loglik(j) Log2(i).loglik(j) Log3(i).loglik(j) ]);            
...
%                 Itr1 = Itr1 + 1;
            elseif(length(Index) > 1)    % Sometimes the array of [Log1 Log2 Log3 Log4] are all "-Inf", hence the Index = [1 2 3 4]
                ...
                SpecialLabel(Itr2,2) = 4;   %<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<CHANGE TO 5 IF 4 CLASSES
                Itr2 = Itr2 + 1;
                EstLabel(Itr1,1) = LabelArray(j);    % True Labels
                EstLabel(Itr1,2) = 4;    % Estimated Labels%<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<CHANGE TO 5 IF 4 CLASSES
                Itr1 = Itr1 + 1;
            end                                        
        end
    end
