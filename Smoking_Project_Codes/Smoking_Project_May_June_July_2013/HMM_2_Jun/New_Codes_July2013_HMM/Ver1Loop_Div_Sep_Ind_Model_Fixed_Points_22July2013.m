...
path2 = char('C:\Users\student\Documents\MATLAB\Smoking_Project_May_June_July_2013\HMM_2_Jun\New_Data_Encoded_Selected_BC_23July2013\ComWhole_Sit_Walk_Sep\');   %%% <<<<<<<<<<<<<< CHANGE HERE AND NEXT LINE
...
load('C:\Users\student\Documents\MATLAB\Smoking_Project_May_June_July_2013\HMM_2_Jun\Waveform_Numbering\ComWhole_Sit_Walk_Sep')    %% <<<<<<<<<<<<<< CHANGE HERE AND NEXT LINE
...
for i = 1 : 1 : length(SubWNos)
    ...
    SubWNos(i).Walk = num2cell(SubWNos(i).Walk);% <<<<<<<<<<<<<<<<< COMMENT THIS IS YOU HAVE 3 FIELDS <<<<<<<<< %
...
while(Sep <= 50)
    ...
    while(Div <= 4)    
        ...
        String1 = [];
        String1 = strcat('EnCod_',num2str(Div),'Div_',num2str(Sep),'pts_VTASts_24Jul13');
        
        ListInfo(Count).Name{1,1} = String1;
        ListInfo(Count).Sep = Sep;
        ListInfo(Count).Div = Div;
...
save(strcat(path2,'TimeInfo1'),'TimeTaken');

save(strcat(path2,'ListInfo'),'ListInfo');
