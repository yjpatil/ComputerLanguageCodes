...
%while(I <= Size_List)
    ...
    [Code11,Code12] = WinbyWinEncode2Ver(VTts,ASts,PtsVT,VtsVT,Actcnt,WinSize,Sep,Div,Activities,11,12); % The last entry is the Figure Number.e.g here 12 = figure(12)
    %%%%%[Code12] = WinbyWinEncode2Ver(VTts,PtsVT,VtsVT,Actcnt,WinSize,Sep,Div,Activities,12);
    %---------------------------------------------------------------------%     
    Actcnt = 2; % Activity # 2 %   Reading
    [Code21,Code22] = WinbyWinEncode2Ver(VTts,ASts,PtsVT,VtsVT,Actcnt,WinSize,Sep,Div,Activities,21,22);
    %%%%%[Code22] = WinbyWinEncode2Ver(VTts,PtsVT,VtsVT,Actcnt,WinSize,Sep,Div,Activities,22);
    %---------------------------------------------------------------------%     
    Actcnt = 3; % Activity # 3 %  Standing
    [Code31,Code32] = WinbyWinEncode2Ver(VTts,ASts,PtsVT,VtsVT,Actcnt,WinSize,Sep,Div,Activities,31,32);
    %%%%%[Code32] = WinbyWinEncode2Ver(VTts,PtsVT,VtsVT,Actcnt,WinSize,Sep,Div,Activities,32);
    %---------------------------------------------------------------------%     
    Actcnt = 4; % Activity # 4 %  Walk Slow
    [Code41,Code42] = WinbyWinEncode2Ver(VTts,ASts,PtsVT,VtsVT,Actcnt,WinSize,Sep,Div,Activities,41,42);
    %%%%%[Code42] = WinbyWinEncode2Ver(VTts,PtsVT,VtsVT,Actcnt,WinSize,Sep,Div,Activities,42);
    %---------------------------------------------------------------------%     
    Actcnt = 5; % Activity # 5 %  Walk Fast
    [Code51,Code52] = WinbyWinEncode2Ver(VTts,ASts,PtsVT,VtsVT,Actcnt,WinSize,Sep,Div,Activities,51,52);
    %%%%%[Code52] = WinbyWinEncode2Ver(VTts,PtsVT,VtsVT,Actcnt,WinSize,Sep,Div,Activities,52);
    %---------------------------------------------------------------------%     
    Actcnt = 6; % Activity # 6 %  Laptop
    [Code61,Code62] = WinbyWinEncode2Ver(VTts,ASts,PtsVT,VtsVT,Actcnt,WinSize,Sep,Div,Activities,61,62);
    %%%%%[Code62] = WinbyWinEncode2Ver(VTts,PtsVT,VtsVT,Actcnt,WinSize,Sep,Div,Activities,62);
    %---------------------------------------------------------------------%     
    Actcnt = 7; % Activity # 7 %  Eat with Hands
    [Code71,Code72] = WinbyWinEncode2Ver(VTts,ASts,PtsVT,VtsVT,Actcnt,WinSize,Sep,Div,Activities,71,72);
    %%%%%[Code72] = WinbyWinEncode2Ver(VTts,PtsVT,VtsVT,Actcnt,WinSize,Sep,Div,Activities,72);
    %---------------------------------------------------------------------%     
    Actcnt = 8; % Activity # 8 %  Eat with Fork
    [Code81,Code82] = WinbyWinEncode2Ver(VTts,ASts,PtsVT,VtsVT,Actcnt,WinSize,Sep,Div,Activities,81,82);
    %%%%%[Code82] = WinbyWinEncode2Ver(VTts,PtsVT,VtsVT,Actcnt,WinSize,Sep,Div,Activities,82);
    %---------------------------------------------------------------------%     
    Actcnt = 9; % Activity # 9 % Walk Outside
    [Code91,Code92] = WinbyWinEncode2Ver(VTts,ASts,PtsVT,VtsVT,Actcnt,WinSize,Sep,Div,Activities,91,92);
    %%%%%[Code92] = WinbyWinEncode2Ver(VTts,PtsVT,VtsVT,Actcnt,WinSize,Sep,Div,Activities,92);
    %---------------------------------------------------------------------%     
    Actcnt = 10; % Activity # 10 % Smoking Sit
    [Code101,Code102] = WinbyWinEncode2Ver(VTts,ASts,PtsVT,VtsVT,Actcnt,WinSize,Sep,Div,Activities,101,102);
    %%%%%[Code102] = WinbyWinEncode2Ver(VTts,PtsVT,VtsVT,Actcnt,WinSize,Sep,Div,Activities,102);
    %---------------------------------------------------------------------%     
    Actcnt = 11; % Activity # 11 % Rest
    [Code111,Code112] = WinbyWinEncode2Ver(VTts,ASts,PtsVT,VtsVT,Actcnt,WinSize,Sep,Div,Activities,111,112);
    %%%%%[Code112] = WinbyWinEncode2Ver(VTts,PtsVT,VtsVT,Actcnt,WinSize,Sep,Div,Activities,112);
    %---------------------------------------------------------------------%     
    Actcnt = 12; % Activity # 12 % Smoking Stand
    [Code121,Code122] = WinbyWinEncode2Ver(VTts,ASts,PtsVT,VtsVT,Actcnt,WinSize,Sep,Div,Activities,121,122);
    %%%%%[Code122] = WinbyWinEncode2Ver(VTts,PtsVT,VtsVT,Actcnt,WinSize,Sep,Div,Activities,122);    
    %---------------------------------------------------------------------% 
    States = 16;Obs = 15;
    
    ActModel = 1;ActTest = 10;
    
    [LogLkhdAct1, HMMAct1] = GetHMM(Code11,States,Obs);
    
    fprintf('\n ======================================================= \n');
    fprintf('\n ========= Test Results for Act1 HMM on ACT10 ========= \n');
    for i = 1 : 1 : length(Code101)
        %j = Array(i);
        loglikOA(i) = dhmm_logprob(Code101{i},HMMAct1.prior, HMMAct1.transmat, HMMAct1.obsmat);
    ...
    for i = 1 : 1 : length(Code11)
        %j = Array(i);
        loglikSA(i) = dhmm_logprob(Code11{i},HMMAct1.prior, HMMAct1.transmat, HMMAct1.obsmat);
    ...
    figure;hold on;
    plot(ActModel*ones(length(loglikSA),1),loglikSA,'+','Color',[1,0,0]);
