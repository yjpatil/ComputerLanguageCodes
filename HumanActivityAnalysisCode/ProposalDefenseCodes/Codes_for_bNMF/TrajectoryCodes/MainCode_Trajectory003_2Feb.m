...
SampleData = 5; 
SamplePlot = 25; % <>----- "After how many iterations do you want to plot"
...
StopSign = 12;  % <>----- "Important:: How many times you want the Target to bump at the end and take reverse turn"

carL = 1.2;
carW = 1.3;

% % ----- Initial assignment of the position of the subject ----- % %
s11 = [2,19,-45];%[X,Y,Angle w.r.t X]   % <>-------- Enter the Starting Point for 1st subject
computecarpoints(s11(1),s11(2),s11(3),'r',carL,carW);
hold on;
% % %
s22 = [19,3,135];%[X,Y,Angle w.r.t X]  % <>-------- Enter the Starting Point for 2nd subject
...
while (s22(1) > 0.0000001 && s22(1) < 19.99999995 && s22(2) > 0.0000001 && s22(2) < 19.9999995)
    ...
    if (rem(i,SampleData)==0)           
        ...
        elseif(XCoG2 >= 19 || YCoG2 <= 2 )
            s22(3) = 135;
            [X2,Y2,XCoG2,YCoG2]=computecarpoints01(s22(1), s22(2), s22(3),'c',carL,carW,0);
            WalkCount2 = WalkCount2 + 1;
        elseif(XCoG2 <= 1 || YCoG2 <= 1 )
            s22(3) = 0;
            [X2,Y2,XCoG2,YCoG2]=computecarpoints01(s22(1), s22(2), s22(3),'c',carL,carW,0);
            WalkCount2 = WalkCount2 + 1;
...
while (s33(1) > 0.0000001 && s33(1) < 19.99999995 && s33(2) > 0.0000001 && s33(2) < 19.9999995)
    ...
    if (rem(i,SampleData)==0)       
        ...
        [X3,Y3,XCoG3,YCoG3]=computecarpoints01(s33(1), s33(2), s33(3),'b',carL,carW,0);  
        
        if(rem(i,SamplePlot)==0)
            [X3,Y3,XCoG3,YCoG3]=computecarpoints01(s33(1), s33(2), s33(3),'b',carL,carW,1);  
        end
...
title('Histogram of Location for Activity03','FontSize',13.0);xlabel('Locations','FontSize',FontSize);
