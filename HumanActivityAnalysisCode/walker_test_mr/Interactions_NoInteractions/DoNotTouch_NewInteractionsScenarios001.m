...
addpath('C:\Users\Yuri\Dropbox\DrBobHaoResearch\Codes\RuiMa_codes\3D_model\walker_test_mr\Main_Functions2')
addpath('C:\Users\student\Dropbox\DrBobHaoResearch\Codes\RuiMa_codes\3D_model\walker_test_mr\Main_Functions')
...
Case = 1;Scenario_choice = 2; % <------------------------------- ******** What kind of Trajectory you want 
if(Case == 1)
...
while(i <= length(pathX1) - stop1 )%%|| j <= length(pathX2) - stop2 || k <= length(pathX3) - stop3) 
    ...
    %--$$-- SUBJECT 2:      
    if(j <= length(pathX2)-5)%% - stop2)  %%subject 2 should stop before subject 1
