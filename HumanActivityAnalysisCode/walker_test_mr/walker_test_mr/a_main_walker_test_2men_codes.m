...
path1_index=[path1_index,length(path1X)];
path2_index=[path2_index,length(path2X)];
restore_codes;
clear human11 human12 human21 human22;
%------------------------------------------------------------
cyl = UnitCylinder(2);
sph = UnitSphere(2);
...
% Head 
L1 = 2;
Head2 = translate(scale(sph,L1/2, L1/2, L1/1.4),0,0,L1+4.5);
Head2.facecolor = [1 0.55 0.55];
Head1=Head2;Head1.facecolor = [0.3 0.3 1];
%---------------------------------------------------------
...
%---------------------------------------------------------
Head2 = combine(Head2,Neck,meys1,meys2);
...
for i = 1:len-1
    ...
    if (path1X(i+1)==path1X(i))&&(path1Y(i+1)==path1Y(i))
        angle11=0;angle12=0;
    end
    if i+1<=len2
        if(path2X(i+1)==path2X(i))&&(path2Y(i+1)==path2Y(i))
            angle21=0;angle22=0;
        end
    else
        angle21=0;angle22=0;
    ...
    % --------------------------------------------------------------------- 
    % ----------------man 1------------------------------------------------
    Arm_left1 = combine(translate(rotateY(Fore_Arm_left,angle12),0,0,-L3), Upper_Arm_left);
    Arm_right1 = combine(translate(rotateY(Fore_Arm_right,-angle12),0,0,-L3), Upper_Arm_right);
    
    Arm_left1 = translate(rotateY(Arm_left1,angle11),0,-L2/2,(5-L3/2)+L3/2);
    Arm_right1 = translate(rotateY(Arm_right1,-angle11),0,L2/2,(5-L3/2)+L3/2);
    
    Leg_left1 = combine(translate(rotateY(Lower_Leg_left,-angle12),0,0,-L7), Upper_Leg_left);
    Leg_right1 = combine(translate(rotateY(Lower_Leg_right,angle12),0,0,-L7), Upper_Leg_right);
    
    Leg_left1 = translate(rotateY(Leg_left1,-angle11),0,-L71,L72+L7/2);
    Leg_right1 = translate(rotateY(Leg_right1,angle11),0,L71,L72+L7/2);
    
    Upper_Body1 = combine(Head1, Shoulder, Arm_left1, Arm_right1, Chest, Weist);
    Lower_Body1 = combine(Hip, Leg_left1, Leg_right1);
    % ----------------man 2------------------------------------------------
    Arm_left2 = combine(translate(rotateY(Fore_Arm_left,angle22),0,0,-L3), Upper_Arm_left);
    Arm_right2 = combine(translate(rotateY(Fore_Arm_right,-angle22),0,0,-L3), Upper_Arm_right);
    
    Arm_left2 = translate(rotateY(Arm_left2,angle21),0,-L2/2,(5-L3/2)+L3/2);
    Arm_right2 = translate(rotateY(Arm_right2,-angle21),0,L2/2,(5-L3/2)+L3/2);
    
    Leg_left2 = combine(translate(rotateY(Lower_Leg_left,-angle22),0,0,-L7), Upper_Leg_left);
    Leg_right2 = combine(translate(rotateY(Lower_Leg_right,angle22),0,0,-L7), Upper_Leg_right);
    
    Leg_left2 = translate(rotateY(Leg_left2,-angle21),0,-L71,L72+L7/2);
    Leg_right2 = translate(rotateY(Leg_right2,angle21),0,L71,L72+L7/2);
    
    Upper_Body2 = combine(Head2, Shoulder, Arm_left2, Arm_right2, Chest, Weist);
    Lower_Body2 = combine(Hip, Leg_left2, Leg_right2);
    % ---------------------------------------------------------------------
    walker1  = combine(Upper_Body1, Lower_Body1);
    walker2  = combine(Upper_Body2, Lower_Body2);
   % ---------------------------------------------------------------------
    walker1 = scale(walker1,0.3,0.3,0.55);
    walker2 = scale(walker2,0.3,0.3,0.55);
    % ---------------------------------------------------------------------
    cla
    % ---------------------------------------------------------------------
    if i+1<=len1
    ...
    if i+1<=len2
        ...
        fill(xfill,yfill,'r')
        walker2  = rotateZ(walker2,angle_direct2);
        walker2  = translate(walker2,path2X(i),path2Y(i),3);
    else
        ...
        fill(xfill,yfill,'r')
        walker2  = rotateZ(walker2,angle_direct2(end));
        walker2  = translate(walker2,path2X(end),path2Y(end),3);
    end
    % --------------------------------------------------------------------- 
    % --------------------------------------------------------------------- 
    ...
    %M(:,i) = getframe;
    angle11 = w1_s(rem(i,length(w1_s))+1);
    angle12 = w2_s(rem(i,length(w1_s))+1);
    
    angle21 = w1_s(rem(i,length(w1_s))+1);
    angle22 = w2_s(rem(i,length(w1_s))+1);
    
    title(codes{i},'Position',[-0.5,-0.5,12],'FontSize',18);
