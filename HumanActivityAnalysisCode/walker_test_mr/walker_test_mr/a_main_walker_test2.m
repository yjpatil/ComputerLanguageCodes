...
pathY = [pathY,(pathY(length(pathY))-0.1)*ones(1,length([14.1:0.1:22]))];
pathZ =-6*ones(1,length(pathY));
angle_direct=atan(([pathY,0]-[0,pathY])./([pathX,0]-[0,pathX]))/pi*180;
%plot(pathX,pathY)
% Head 
L1 = 2;
...
%for i = 1:10
   ...
   % ---------------------------------------------------------------------
    %angle_direct=atan((pathY(i+1)-pathY(i))/(pathX(i+1)-pathX(i)))/pi*180;
    %walker = scale(walker,0.25,0.25,0.25);
    walker  = rotateZ(walker,angle_direct(i+1));
   ...
   % --------------------------------------------------------------------- 
    ...
    grid on
    drawnow
    set(gca,'xlim',[-1 19],'ylim',[-1 22],'zlim',[-5 9]);
    %axis equal
