function [confmatrix] = cfmatrix(actual, predict, classlist, per, printout)
...
for i = 1:n_class 
    ...
    temps6 = strcat(temps6,sprintf(' |   %2.2f    ',Sensitivity(i) ));
    % Specificity ( mostly used in 2 class problems )=
    % TrueNegativeRate(class)
    % = TN(class) / ( TN(class) + FP(class) )
    Specificity(i) = TPFPFNTN(4, i) / ( TPFPFNTN(4, i) + TPFPFNTN(2, i) );
    temps7 = strcat(temps7,sprintf(' |   %2.2f    ',Specificity(i) ));
end 
