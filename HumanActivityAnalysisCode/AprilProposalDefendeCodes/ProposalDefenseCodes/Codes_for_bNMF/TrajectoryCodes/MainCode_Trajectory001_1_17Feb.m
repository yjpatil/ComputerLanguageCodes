...
StopSign = 5;  % <>----- "Important:: How many times you want the Target to bump at the end and take reverse turn"

carL = 1.2;
carW = 1.3;

% % ----- Initial assignment of the position of the subject ----- % %
s11 = [17,17,225];%[X,Y,Angle w.r.t X]   % <>-------- Enter the Starting Point for 1st subject
%computecarpoints1(s11(1),s11(2),s11(3),carL,carW);
hold on;
s22 = [2.5,17.5,-45];%[X,Y,Angle w.r.t X]  % <>-------- Enter the Starting Point for 2nd subject
...
load('LDPCmat03.mat');
%LPDCmat = LDPCmat03;%gen_ldpc(D1,Blocks);
...
while (s22(1) > 0.0000001 && s22(1) < 19.99999995 && s22(2) > 0.0000001 && s22(2) < 19.9999995)
    ...
    if (rem(i,SampleData)==0)    
        ...
        CodeRec2(it2,:) = Code2';
        it2 = it2 + 1;
        if(XCoG2 >= 18.5 || YCoG2 <= 2 )
            s22(3) = 135;
            [X2,Y2,XCoG2,YCoG2]=computecarpoints002(s22(1), s22(2), s22(3),carL,carW,0);
            WalkCount2 = WalkCount2 + 1;
        elseif(XCoG2 <= 2 || YCoG2 >= 18 )
            s22(3) = -45;
            [X2,Y2,XCoG2,YCoG2]=computecarpoints002(s22(1), s22(2), s22(3),carL,carW,0);
            WalkCount2 = WalkCount2 + 1;
...
%clearvars -except V CodeRec1 CodeRec2 LPDCmat
...
% timelimit = 10000;
% 
% %[W,H] = fnmf2(V,Winit,Hinit,tol,timelimit,maxiter)
% VV = V;
% real_num = 1;
% base_num = 16;
% prior = 1;
% reg = 1;
% [context_prior,tracking_prediction] = BNMF_main(V,VV,base_num,real_num,prior,reg)
