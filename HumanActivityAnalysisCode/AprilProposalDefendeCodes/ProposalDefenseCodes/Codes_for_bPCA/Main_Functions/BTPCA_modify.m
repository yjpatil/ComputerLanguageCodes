function [LogL,MoFA,qcn,RelChange] = BTPCA_modify(X,L,C,ti,dia,model)
...
%
%Input:
% X         : data matrix (N (# of images)data points in D (# of pixels) dimensions as columns)
% L         : latent dimensionality
% C         : number of mixture components (i.e. # of clusters/classes)
...
% >>[LogL,MoFA]=BTPCA(x,1,1,1,1); % size(x) = 72 rows, 96 columns, 64
% images (72x96x64)
...
epsilon       = 1e-4;                                     % relative log-likelihood increase threshold for convergence
...
[D,N]         = size(X); % check the size of each image %

%handle image data, precompute ffts and reseve memory

% ----------------------------------------------- %
% *** Check if the input data is a 3D matrix *** %
if (ndims(X)==3); 
    img=1;
else
    img=0;
...
% ----------------------------------------------- %
% *** check if the Dimensions of the model and the test data are of same size, each row
% length == D *** %
if exist('model')
    if size(model.W,1)~=D 
        if size(model.W,1)==N*D
            img=1;
        else
...
% ----------------------------------------------- %
% ** This is a fast calculation step where the expected aligned image is
% calculated for given image considering all possible shifts ***%

if(img)% if imag == 1, i.e. the input data X is 3D matrix
    [nHeight,nWidth,N]=size(X);% get actual dimensions of each image %
    D = [nHeight*nWidth]; % This total number of bits present, or total number of PIXELS
    if(ti) % ti: transformation invariant (integrate over all possible 2D image shifts) if 1 (default=0)
        Xfft   = zeros(nHeight,nWidth,N);
        conjXfft = zeros(nHeight,nWidth,N);
        for t = 1:N;
            Xfft(:,:,t) = fft2(X(:,:,t));
            conjXfft(:,:,t) = conj(Xfft(:,:,t));
        end
    end
    X=reshape(X,[],N);% make flat % Plot this Image Samples in Black and White
end
% ----------------------------------------------- %
% **** Show plotting of results **** %
if ~exist('dia')
    dia=0;
end
if dia
...
if (dia>=3)
    figure(2)
    showMoFAImages(MoFA,nHeight,nWidth);
end
...
for iter=1:max_iters; % EM loops
    ...
    if ~ti
        ...
        for c = 1:C
            ...
            LLcn(c,:) = sum( X.*T(:,:,c) - log(1+exp(T(:,:,c)))  , 1 );  % (19)  %no of operations: c*[N*D]
        end
        ...
        qcn    = exp(qcn)+realmin; % equation (19)
        qcn    = qcn ./ repmat(sum(qcn,1),C,1); % equation (13)
        LLn    = sum(qcn.*(-log(qcn+realmin) + LLcn),1);% equation (11)     this follows from the tight lower bound qc*log(pc*Pc/qc)      
        
        LogL(iter) = sum(LLn);  % Sum over all N sample images
