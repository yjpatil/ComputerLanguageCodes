function[SUB,Counter] = fWbyWNumVer1(SUB,I,Counter,SignalVT,SignalAS,Peaks,Valleys,Actcnt,WinSize,Sep1,Div,Activities,FigVT,FigAS,LenOption)
...
% %     figure(FigVT);hold on;
    ...
    if(strcmp('FixLen',LenOption))
        SepVT = round(length(VTts_temp)/Sep1); %% <<------------------------ Here the changes were made!!!
    elseif(strcmp('VarLen',LenOption))
        SepVT = Sep1;
    end
    
    %length(VTts_temp);
    
    [CodeVT] = EncodedSeq(VTts_temp,SepVT,Div);  
    SUB(I).CodeVT{Counter} = CodeVT;
...
% %     figure(FigAS);hold on;
    ...
    if(strcmp('FixLen',LenOption))
        SepAS = round(length(ASts_temp)/Sep1); %% <<------------------------ Here the changes were made!!!
    elseif(strcmp('VarLen',LenOption))
        SepAS = Sep1;
    end
    [CodeAS] = EncodedSeq(ASts_temp,SepAS,Div);  
    SUB(I).CodeAS{Counter} = CodeAS;
